...
iteration = 0
with con:
    # res = con.execute("SELECT * FROM project_name LIMIT 20")
    res = con.execute("SELECT * FROM project_name WHERE post_id >= 8084")
    for row in res:
        time.sleep(5)
        post_id = row[0]
        ...
        print(download_url)
        # pdf = pdfkit.from_url(download_url, final_title+'.pdf', options=options)
