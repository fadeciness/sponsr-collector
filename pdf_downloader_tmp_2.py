...
with con:
    ...
    res = con.execute("SELECT * FROM project_name ORDER BY post_id LIMIT 1")
    for row in res:
        # time.sleep(5)
        post_id = row[0]
        ...
        for im in images:
            im['src'] = str(host) + str(im['src'])
        # print(soup)

        # with open(final_title+'.html', 'w', encoding="utf-8") as file:
        #     file.write(resp.text)

        # pdfkit.from_url(download_url, final_title + '.pdf', options=options, verbose=True)
        # pdfkit.from_string(str(soup), final_title + '.pdf', options=options, verbose=True)
        ...
        pdfkit.from_string(str(soup), final_title + '.pdf', options=options, verbose=True)
        # pdfkit.from_file(final_title+'.html', final_title + '.pdf', options=options, verbose=True)
        # pdfkit.from_file(final_title+'.html', final_title + '.pdf', verbose=True)
        print('finish')
